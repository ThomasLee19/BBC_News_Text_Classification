...
class GRUClassifier(nn.Module):
    def __init__(self, vocab_size, embed_dim, hidden_dim, num_classes,
        ...
        # GRU层：比LSTM参数少约25%，计算效率更高
        # 使用reset gate和update gate替代LSTM的三个门
        self.gru = nn.GRU(
            embed_dim,
            hidden_dim,
            num_layers=num_layers,
            bidirectional=bidirectional,
            dropout=dropout if num_layers > 1 else 0,  # 多层GRU才使用dropout
            batch_first=True
        )
    ...
    def forward(self, x):
        ...
        if self.bidirectional:
            # 拼接双向GRU的最后隐藏状态（forward和backward）
